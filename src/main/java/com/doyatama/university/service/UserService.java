...
public class UserService {
    ...
    return userRepository.save(user); // Di sini ID akan otomatis tergenerate lewat logic di repository
}

public User saveUserFromRequest(UserRequest userRequest) throws IOException {
    User user = new User();
    user.setName(userRequest.getName());
    user.setUsername(userRequest.getUsername());
    user.setEmail(userRequest.getEmail());
    user.setPassword(userRequest.getPassword()); // idealnya kamu hash password dulu di sini
    user.setRoles(userRequest.getRoles());
    user.setCreatedAt(Instant.now()); // ✅ fix disini
